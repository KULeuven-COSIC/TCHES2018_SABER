...
//*******************************************************************client matrix-vector multiplication ends************************************
	...
	//VectorMul(pk,skpv1,(SABER_P-1),vprime);
	VectorMul(pk,skpv1,res[0]);
...
void indcpa_kem_dec(const unsigned char *sk, const unsigned char *ciphertext, unsigned char message_dec[])
{
	...
	uint8_t mask_ar[SABER_SCALEBYTES_KEM];
	...
	for(i=0;i<SABER_SCALEBYTES_KEM;i++){
		mask_ar[i]=ciphertext[SABER_POLYVECCOMPRESSEDBYTES+i];
	}

	ExtractKeyBits(v,mask_ar,message_dec_unpacked);
...
void MatrixVectorMul(const unsigned char *seed, uint16_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], int8_t transpose){
	...
	if(transpose==1){
		...
		for(i=0;i<SABER_K;i++){
			for(j=0;j<SABER_K;j++){
				GenMatrix_poly(temp_ar, seed, i+j);
				toom_cook_4way_mem(temp_ar, skpv[j], res[i]);
			}
		}
	}
